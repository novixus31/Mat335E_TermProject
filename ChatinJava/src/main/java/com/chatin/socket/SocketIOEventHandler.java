...
public class SocketIOEventHandler implements CommandLineRunner {
    ...
    private DataListener<Map> onRequestQR() {
        return (client, data, ackSender) -> {
            ...
            logger.info("QR requested for account: {} (token present: {})", accountId, token != null);

            // Client'ı account room'una kat — event'leri alabilmesi için kritik
            if (accountId != null) {
                client.joinRoom("account_" + accountId);
                logger.debug("Client {} joined room account_{}", client.getSessionId(), accountId);
            }

            try {
                whatsAppManager.startConnection(accountId, null);
            } catch (Exception e) {
                logger.error("Failed to start QR via socket: {}", e.getMessage());
                client.sendEvent("qr_error", Map.of("message", "Failed to start connection: " + e.getMessage()));
            }
        };
    }
    ...
    private DataListener<Map> onStopWhatsApp() {
        return (client, data, ackSender) -> {
            String accountId = (String) data.get("accountId");
            logger.info("Stop WhatsApp requested for account: {}", accountId);

            whatsAppManager.stopConnection(accountId);

            // Client'a doğrudan connection_closed gönder
            client.sendEvent("connection_closed", Map.of());

            // Room'dan ayrıl
            if (accountId != null) {
                client.leaveRoom("account_" + accountId);
                logger.debug("Client {} left room account_{}", client.getSessionId(), accountId);
            }
        };
    }
