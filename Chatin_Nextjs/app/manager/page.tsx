  ...
  useEffect(() => {
    async function fetchDashboardData() {
      ...
      try {
        ...
        // Fetch overview stats (Manager-specific endpoint)
        const overviewResponse = await fetch(`${backendUrl}/api/dashboard/manager/overview`, {
          headers: {
        ...
        if (overviewData.success && overviewData.statistics) {
          ...
          setStats({
            ...
            regularUsers: statistics.users.regularUsers || statistics.users.total
          });
        }

        // Fetch health data (Manager-specific endpoint)
        const healthResponse = await fetch(`${backendUrl}/api/dashboard/manager/health`, {
          headers: {
            'Authorization': `Bearer ${session.user.accessToken}`
          }
        });
  ...
  const statCards = [
    {
      ...
      subtitle: `${stats.managers} managers, ${stats.regularUsers} users`,
      icon: Users,
      color: 'blue',
    ...
    <div className="p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Link
          ...
          className="bg-white rounded-lg border border-gray-200 p-4 hover:shadow-lg transition-all hover:border-indigo-300 group"
        >
          <div className="flex items-center justify-between mb-3">
            <MessageSquare className="w-6 h-6 text-indigo-600" />
            <ArrowRight className="w-4 h-4 text-gray-400 group-hover:text-indigo-600 transition-colors" />
          </div>
          <h3 className="text-base font-semibold text-gray-900 mb-1">View All Chats</h3>
          <p className="text-xs text-gray-600">Access all company conversations</p>
        </Link>
