...
public class BinaryDecoder {
    ...
    private static String readString(int tag, byte[] buffer, int[] indexRef) throws Exception {
        ...
        return switch (tag) {
            ...
            case BinaryConstants.TAGS.INTEROP_JID -> readInteropJid(buffer, indexRef);
            case BinaryConstants.TAGS.FB_JID -> readFbJid(buffer, indexRef);
    ...
    private static String readInteropJid(byte[] buffer, int[] indexRef) throws Exception {
        String user = readString(readByte(buffer, indexRef), buffer, indexRef);
        int device = readInt(2, buffer, indexRef);
        int integrator = readByte(buffer, indexRef);
        return user + ":" + device + ":" + integrator + "@interop";
    }

    private static String readFbJid(byte[] buffer, int[] indexRef) throws Exception {
        String user = readString(readByte(buffer, indexRef), buffer, indexRef);
        int device = readInt(2, buffer, indexRef);
        return user + ":" + device + "@fb";
    }
